							...
							<table id="datatablesSimple">
								...
								<tbody>
									...
									<form class="form" action="#" method="POST">
										...
										<%
										ArrayList id = new ArrayList((ArrayList) request.getAttribute("id"));/*編號*/
										...
										ArrayList date = new ArrayList((ArrayList) request.getAttribute("date"));/*編號*/
										int z = name.size();
										%>
										<%
										for (int n = 0; n < z; n++) {
										...
										<tr>
											...
											<td><%=c_sellerID.get(n)%></td>
											<td><%=name.get(n)%></td>
											<td><%=performance.get(n)%></td>
											<td><%=date.get(n)%></td>
											<td><%=time.get(n)%></td>
											<td><%=photo.get(n)%></td>
											<td><%=film.get(n)%></td>
											<td>
												<div class="cre">
													<a href="<c:url value='<%="/WANG_MERGE/"+id.get(n)+"/" %>'/>" class='btna'>修改</a>
													<div>&emsp;</div>
													<a href="<c:url value='<%="/WANG_DELTE/"+id.get(n)+"/" %>'/>" class='btna'>刪除</a>
													<div>&emsp;</div>
												</div>
											</td>
										</tr>
				...
				<div class="container-fluid px-4">
					<div
						...
						<div class="text-muted">Copyright &copy;
							YA!NightMarketCollection夜總匯 2021</div>
						<div>
							<a href="#">隱私政策</a> &middot; <a href="#">服務約定 &amp; 條款</a>
						</div>
					</div>
				</div>
			</footer>
